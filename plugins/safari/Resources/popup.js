"use strict";
(() => {
  // src/popup.ts
  function send(msg) {
    return new Promise((resolve) => chrome.runtime.sendMessage(msg, resolve));
  }
  var elUri = document.getElementById("current-uri");
  var elBadge = document.getElementById("server-badge");
  var elLinksList = document.getElementById("links-list");
  var elLinkTarget = document.getElementById("link-target");
  var elLinkNote = document.getElementById("link-note");
  var elServerUrl = document.getElementById("server-url");
  var elStatus = document.getElementById("status");
  var currentUri = "";
  function setStatus(msg, ms = 3e3) {
    elStatus.textContent = msg;
    if (ms > 0) setTimeout(() => {
      elStatus.textContent = "";
    }, ms);
  }
  async function init() {
    chrome.storage.local.get(["serverUrl"], (r) => {
      elServerUrl.value = r.serverUrl ?? "http://127.0.0.1:2701";
    });
    const { available } = await send({ type: "probe" });
    elBadge.textContent = available ? "online" : "offline";
    elBadge.className = `server-badge ${available ? "online" : "offline"}`;
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab?.url) return;
    const { uri } = await send({
      type: "buildUri",
      payload: { url: tab.url }
    });
    currentUri = uri;
    elUri.textContent = uri;
    elUri.title = uri;
    if (!available) {
      elLinksList.innerHTML = '<li class="empty">Start hk serve to load links.</li>';
      return;
    }
    await loadLinks();
  }
  async function loadLinks() {
    if (!currentUri) return;
    const res = await send({
      type: "listLinks",
      payload: { uri: currentUri }
    });
    if (!res.ok || !res.data) {
      elLinksList.innerHTML = `<li class="empty">${res.error ?? "Error loading links"}</li>`;
      return;
    }
    if (res.data.length === 0) {
      elLinksList.innerHTML = '<li class="empty">No links yet.</li>';
      return;
    }
    elLinksList.innerHTML = res.data.map((l) => {
      const other = l.uri_a === currentUri ? l.uri_b : l.uri_a;
      const note = l.note ? ` \u2014 ${l.note}` : "";
      return `<li title="${other}${note}">${other}${note}</li>`;
    }).join("");
  }
  document.getElementById("btn-copy-uri").addEventListener("click", async () => {
    if (!currentUri) return;
    await navigator.clipboard.writeText(currentUri);
    setStatus("\u2713 Copied to clipboard");
  });
  document.getElementById("btn-create-link").addEventListener("click", async () => {
    const target = elLinkTarget.value.trim();
    if (!target) {
      setStatus("Enter a target URI first.");
      return;
    }
    if (!currentUri) {
      setStatus("No page URI available.");
      return;
    }
    const res = await send({
      type: "createLink",
      payload: { uriA: currentUri, uriB: target, note: elLinkNote.value.trim() || void 0 }
    });
    if (res.ok) {
      elLinkTarget.value = "";
      elLinkNote.value = "";
      setStatus("\u2713 Link created");
      await loadLinks();
    } else {
      setStatus(`Error: ${res.error}`);
    }
  });
  document.getElementById("btn-save-settings").addEventListener("click", async () => {
    const url = elServerUrl.value.trim();
    if (!url) return;
    await send({ type: "updateSettings", payload: { serverUrl: url } });
    setStatus("\u2713 Settings saved \u2014 reopen popup to reconnect");
  });
  init().catch((e) => setStatus(`Error: ${e.message}`));
})();
//# sourceMappingURL=popup.js.map
