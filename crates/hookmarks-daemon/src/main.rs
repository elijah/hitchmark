...
#[cfg(target_os = "linux")]
mod linux {
    use hookmarks_core::{LinkStore, UriType};
    use std::path::PathBuf;
    use zbus::{connection, interface};

    /// DBus service struct — holds state shared across method calls.
    struct HookmarksDaemon {
        store: LinkStore,
    }

    impl HookmarksDaemon {
    ...
    impl HookmarksDaemon {
        ...
        async fn file_to_uri(&self, path: String) -> zbus::fdo::Result<String> {
            ...
            let uri = hookmarks_core::HookUri {
    ...
    pub async fn run() -> anyhow::Result<()> {
        let daemon = HookmarksDaemon::new()?;

        let _conn = connection::Builder::session()?
            .name("org.not_hookmarks.Daemon")?
            .serve_at("/org/not_hookmarks/Daemon", daemon)?
            .build()
            .await?;

        eprintln!("Hookmarks daemon started — DBus name: org.not_hookmarks.Daemon");
